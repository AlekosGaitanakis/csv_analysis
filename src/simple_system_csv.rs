...
use std::{error::Error, fs::{self, File, OpenOptions}, io::{self}, usize};
...
pub fn input_path_file(what_to_do: usize) -> String{
    ...
    if path_to_save == "temp_file_for_sorting" {
        println!("\nThis name is used in functions and cannot be used. Try another name.\n");
        input_path_file(what_to_do);
    }
